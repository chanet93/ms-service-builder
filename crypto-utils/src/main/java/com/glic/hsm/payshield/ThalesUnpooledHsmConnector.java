...
public class ThalesUnpooledHsmConnector implements HsmConnector {
   ...
   private HsmConnectionHandler connection;
   ...
   public ByteBuffer send(ByteBuffer payload) throws HsmException {
      ...
      try {
         ...
         LOGGER.log(Level.INFO, "ResponseByteBuffer: {0} ms.", responseByteBufferAllTime);
         ...
         LOGGER.log(Level.INFO, "Elapsed HSM command time: {0} ms.", elapsed);
      }
   }
   ...
   public HsmConnectionHandler getConnection() {
      return connection;
   }

   public void setConnection(HsmConnectionHandler connection) {
