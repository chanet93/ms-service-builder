...
public enum ETimeZone {
   ...
   private static final Map<String, ETimeZone> timezones = new HashMap<>();

   private static final Map<String, ETimeZone> names = new HashMap<>();

   private static final String KEY_SUFFIX = ETimeZone.class.getSimpleName().toUpperCase(Locale.ROOT) + ".";

   private final String timeZoneCode;

   static {
      for (ETimeZone tz : values()) {
         timezones.put(tz.getTimeZoneCode().toLowerCase(Locale.ROOT), tz);
         names.put(tz.name(), tz);
      }
   }

   ETimeZone(String timeZoneCode) {
   ...
   public ZoneId getZoneId(){
      return ZoneId.of(this.timeZoneCode);
   }

   public static ETimeZone getTimeZone(String timeZone) {
      return ETimeZone.valueOf(timeZone);
   }

   public static boolean isValid(String timeZone) {
      return names.get(timeZone) != null;
   }

   public static ETimeZone getByTimeZoneCode(String zoneCode) {
      if (zoneCode != null) {
         return timezones.get(zoneCode.toLowerCase(Locale.ROOT));
      }
      return null;
   }

   public static ETimeZone getByName(String name) {
      if (name != null) {
